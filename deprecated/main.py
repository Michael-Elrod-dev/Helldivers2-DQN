...
def train(args, network, logger):
    ...
    for step in range(1, args.max_steps + 1):
        ...
        # Process the image
        processed_image = preprocess_image(image_path, args.image_size)
        ...
        loss, accuracy, magnitude, lr = network.step(processed_image, predicted_label, reward)
        ...
        if step % args.BATCH_SIZE == 0:
            ...
            print(f'\rStep: {step}\tEpsilon: {eps:.2f}\tBatch Accuracy: {batch_accuracy:.2f}\tAvg. Accuracy: {avg_accuracy:.2f}\tAvg. Loss: {avg_loss:.4f}', end='')
            if step % (args.BATCH_SIZE * 100) == 0:
                print(f'\rStep: {step}\tEpsilon: {eps:.2f}\tBatch Accuracy: {batch_accuracy:.2f}\tAvg. Accuracy: {avg_accuracy:.2f}\tAvg. Loss: {avg_loss:.4f}')
